...
@Module({
        ...
        MongooseModule.forFeatureAsync([
            {
                ...
                useFactory: (): BaseSchema<AuditLogModel> =>
                    AuditLogSchema.set('collection', CollectionNames.auditLogs),
            },
        ]),
        UserModule,
    ],
    providers: [
        ...
        },
        {
            ...
            useFactory(repository: AuditLogRepository): AuditLogService {
                return new AuditLogServiceImpl(repository);
            },
            inject: [Symbols.domain.auditLog.auditLogRepository],
        },
        {
            provide: Symbols.domain.auditLog.logEnricherService,
            useFactory(userRepository: UserRepository): LogEnricherService {
                return new LogEnricherServiceImpl(userRepository);
            },
            inject: [Symbols.domain.user.userRepository],
        },
    ],
    exports: [
        Symbols.domain.auditLog.auditLogRepository,
        Symbols.domain.auditLog.auditLogService,
        Symbols.domain.auditLog.logEnricherService,
    ],
})
