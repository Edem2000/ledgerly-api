...
export class MongooseRepository<T extends BaseModel, E extends BaseEntity<T>> implements Repository<T, E> {
    ...
    private async *asyncIteratorFromCursor(cursor: Cursor<T>): AsyncIterableIterator<E> {
    ...
    public async create(entity: E): Promise<E> {
        try {
            const entityWithModel = this.defineModel(entity, (entity.toJSON() as T).id);
    ...
    public async getByQuery(query: FilterQuery<T>, skip = 0, limit = 0, sort?: SortQuery<T>): Promise<E[]> {
        try {
    ...
    public async getAllSorted(sort: SortQuery<T>, skip = 0, limit = 0): Promise<E[]> {
        try {
    ...
    public getStream(query: FilterQuery<T>, sort?: SortQuery<T>): AsyncIterableIterator<E> {
    ...
    public async updateById(id: Identifier, update: UpdateQuery<T>): Promise<E | null> {
        try {
            const doc = await this.model.findByIdAndUpdate(id, update, { new: true, lean: true }).exec();
            return this.toEntity(doc as T);
        } catch (error) {
            throw new MongoUpdateError(error);
        }
    ...
    public async updateOne(filter: FilterQuery<T>, update: UpdateQuery<T>): Promise<E | null> {
        try {
            const doc = await this.model.findOneAndUpdate(filter, update, { new: true, lean: true }).exec();
            return this.toEntity(doc as T);
        } catch (error) {
            throw new MongoUpdateError(error);
        }
    ...
    public async updateByIds(ids: Identifier[], update: UpdateQuery<T>): Promise<E[]> {
        try {
            const objectIds = ids.map(identifierToObjectId);
    ...
    public async updateMany(filter: FilterQuery<T>, update: UpdateQuery<T>): Promise<E[]> {
        try {
            ...
            const updatedDocs = await this.model.find(filter).lean().exec();
            return this.toEntities(updatedDocs as T[]);
        } catch (error) {
            throw new MongoUpdateError(error);
        }
    ...
    public async bulkUpdate(toUpdate: BulkUpdate<T>[], upsert = true): Promise<void> {
        try {
            ...
            await this.model.bulkWrite(bulkOps as unknown as AnyBulkWriteOperation<any>[]);
        } catch (error) {
            throw new MongoUpdateError(error);
        }
    }
    ...
    public async transactionalOperation(operations: () => Promise<void>): Promise<void> {
